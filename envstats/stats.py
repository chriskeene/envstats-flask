...
def create_solar_chart1(df):
    ...
    ax = df.plot(kind='line', grid=1, fontsize=10, x="date", label="Solar")
    plt.legend()
    plt.style.use('ggplot') 
    plt.xlabel("Month",  size = 10)
    plt.ylabel("Solar energy output", size = 10)
    plt.title("UK Solar output since 2021")
...
def create_solar_chart2(df):
    ...
    plt.style.use('ggplot') 
    plt.xlabel('Month', size = 10)
    plt.ylabel('Solar output', size = 10)
    for y in df4:
...
def solarstats2():
    output = {
        "title": "UK PV Solar every output2",
    }
    df = get_solar_data()
    # print table 
    df5 = create_solar_table1(df)
